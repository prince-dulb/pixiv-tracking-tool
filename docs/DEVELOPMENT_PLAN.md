...
| v0.0.7 | IPv6 外网访问 + 端口设置 + Bug 修复 + 卡片角标 | 🔧 开发中 |
| v0.0.8 | 自定义排序 + Tag 图表 + 备份还原 + 工具页 | 📋 待规划 |
| v0.0.9 | 应用内更新 | 📋 待规划 |
| v0.1.0 | 系统托盘 | 📋 待规划 |
| v0.1.1 | 图片翻译 | 📋 待规划 |
...
- 双 socket 架构：分别创建 IPv4（`0.0.0.0`）和 IPv6（`::`，`IPV6_V6ONLY`）socket，Windows 双栈兼容
- 启动时控制台打印可访问链接：`http://localhost:PORT` + `http://<局域网IPv4>:PORT` + `http://[<IPv6地址>]:PORT`
- 修复 `run.py` 硬编码端口 8000 的问题

### 3.2 端口设置

设置页新增端口号输入框，修改后重启服务生效。

### 3.3 Bug 修复：多p图页码排序混乱

`tracker.py` 文件收集时 `sorted()` 字典序 → 自然排序（`_natsort_key`），修复 ≥10 页时 `_p0, _p1, _p10, _p2` 的乱序问题。

### 3.4 多p作品卡片角标

- 卡片缩略图左下角显示页数角标（`page_count > 1` 时），样式对称于右下角收藏按钮
- 漫画类型（`type == 'manga'`）额外包裹主题色边框
...
### 4.2 筛选面板新增"排序" section

复用现有手风琴面板模式，tag chip 选择排序字段和方向。
...
- PyInstaller `console=False` → 无命令行窗口
- `pystray` 托盘图标，右键菜单：打开浏览器 / 设置 / 退出
- 控制台输出 → 日志文件
- 启动后 Web 服务后台运行
...
- 集成 manga-image-translator（GPL-3.0，子进程 CLI 调用）
- 详情页"翻译"按钮 → OCR 文字检测 → 日→中翻译 → 渲染回填
- 首次使用时下载模型（~1-2GB）
